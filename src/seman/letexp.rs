...
fn tipar_decs(decs: Vec<Dec>, type_env: TypeEnviroment, value_env: ValueEnviroment, pos: Pos) -> Result<(TypeEnviroment, ValueEnviroment), TypeError> {
    ...
    fn tipar_dec_variable(_VarDec {name, typ, init, ..}: _VarDec, type_env: TypeEnviroment, value_env: ValueEnviroment, pos: Pos) -> Result<(TypeEnviroment, ValueEnviroment), TypeError> {
        let mut new_value_env = value_env.clone();
        let init_type = tipar_exp(*init, type_env.clone(), value_env.clone())?;
        ...
        Ok((type_env, new_value_env))
    }

    fn tipar_decs_bloque_funciones(decs: Vec<_FunctionDec>, type_env: TypeEnviroment, value_env: ValueEnviroment, pos: Pos) -> Result<(TypeEnviroment, ValueEnviroment), TypeError> {
        ...
        let mut new_value_env = value_env;
    ...
    fn tipar_decs_bloque_tipos(decs: Vec<_TypeDec>, type_env: TypeEnviroment, value_env: ValueEnviroment, pos: Pos) -> Result<(TypeEnviroment, ValueEnviroment), TypeError> {
        let mut new_type_env = type_env;
        let mut new_value_env = value_env;
    ...
    for dec in sorted_decs {
        let (new_type_env, new_value_env) = match dec {
            VarDec(vd) => match tipar_dec_variable(vd, new_type_env.clone(), new_value_env.clone(), pos) {
                Ok((tenv, venv)) => (tenv, venv),
                Err(type_error) => return Err(type_error),
            },
            FunctionDec(fd) => match tipar_decs_bloque_funciones(fd, new_type_env.clone(), new_value_env.clone(), pos) {
                Ok((tenv, venv)) => (tenv, venv),
                Err(type_error) => return Err(type_error),
            },
            TypeDec(td) => match tipar_decs_bloque_tipos(td, new_type_env.clone(), new_value_env.clone(), pos) {
                Ok((tenv, venv)) => (tenv, venv),
                Err(type_error) => return Err(type_error),
            },
        };
...
pub fn tipar(exp: Exp, type_env: TypeEnviroment, value_env: ValueEnviroment) -> Result<Tipo, TypeError> {
    match exp {
        Exp {node: _Exp::LetExp {decs, body}, pos} => {
            let (new_type_env, new_value_env) =  match tipar_decs(decs, type_env, value_env, pos) {
                Ok((tenv, venv)) => (tenv, venv),
                Err(type_error) => return Err(type_error),
            };
