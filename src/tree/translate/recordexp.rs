...
pub fn trans_exp(
    ...
    match node {
        Exp::Record { fields, .. } => {
            ...
            let (mut fields_exps, fields_level, fields_frags) = fields
                .iter()
            ...
            let init_statement = seq(formals
                ...
                .map(|(name, _, order)| {
                    Move!(
                        MEM(Box::new(plus!(
                            GLOBAL(STACK_POINTER.to_string()),
            ...
            // ))
            Ok((
                // Copy inits to the stack
                // Call runtime
                // Return the resulting memory address
                ESEQ(
                    Box::new(init_statement),
                    Box::new(external_call(runtime_label.to_string(), vec![CONST(fields_exps.len().try_into().unwrap())]))
                ),
                fields_level, fields_frags
            ))
        }
