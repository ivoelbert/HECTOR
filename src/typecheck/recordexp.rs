...
pub fn typecheck(exp: &Exp, type_env: &TypeEnviroment, value_env: &ValueEnviroment) ->  Result<TigerType, TypeError> {
    let tipar_fields = |args: &Vec<(Symbol, Box<Exp>)>| -> HashMap<Symbol, Result<TigerType, TypeError>> {
        ...
        match record_type {
            TigerType::TRecord(formals, type_id) => {
                ...
                if field_types.is_empty() {
                    Ok(TigerType::TRecord(formals, type_id))
                } else {
                    Err(TypeError::TooManyArguments(*pos))
                }
            },
            TigerType::TUnit | TigerType::TNil | TigerType::TInt(..) | TigerType::TString | TigerType::TArray(..) | TigerType::TipoInterno(..) => Err(TypeError::NotRecordType(*pos)),
        }
    }
